...
WORKING_VIDEOS = {
    ...
    "marketing_stats": "MunG48LH_hs",  # Statistics for Marketing
    "ab_testing": "Bu7I_q9cYQ4",  # A/B Testing Explained
    "engineering_stats": "n8e3-vw_TbU",  # Statistics for Various Fields
}
...
def display_resources(career_key):
    ...
    if career_key in CAREER_RESOURCES:
        ...
        youtube_links = []
        for name, url in resources.items():
            if "YouTube:" in name and "youtube.com" in url:
                youtube_links.append((name, url))
        
        if youtube_links:
            for name, url in youtube_links:
                video_title = name.replace("📺 YouTube: ", "")
                st.markdown(f"""
                <div class='video-link'>
                    ▶️ <a href='{url}' target='_blank'>{video_title}</a>
                </div>
                """, unsafe_allow_html=True)
...
def main():
    ...
    with st.sidebar:
        ...
        if slide["type"] in ["intro", "career"] and "resources" in slide["content"]:
            ...
            if career_key in CAREER_RESOURCES:
                resources = CAREER_RESOURCES[career_key]
                # Show first YouTube video if available
                youtube_videos = [(name, url) for name, url in resources.items() if "YouTube:" in name]
                if youtube_videos:
                    first_name, first_url = youtube_videos[0]
                    st.markdown(f"**Featured Video:** [{first_name.replace('📺 YouTube: ', '')}]({first_url})")
                    st.caption("Click to watch the video!")
        ...
        st.markdown("[Statistics for Beginners](https://www.youtube.com/watch?v=kyjlxsLW1Is)")
    ...
    if slide["type"] == "title":
        ...
        with st.expander("🎯 See Real-World Statistics Examples", expanded=True):
            ...
            with col1:
                ...
                - [Statistics for Beginners](https://www.youtube.com/watch?v=kyjlxsLW1Is)
                - [College Board AP Statistics](https://apcentral.collegeboard.org/courses/ap-statistics)
                """)
            with col2:
        ...
        <div class='video-link'>
            ▶️ <a href="https://www.youtube.com/watch?v=kyjlxsLW1Is" target="_blank">Statistics for Beginners</a>
        </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if "youtube_video" in slide["content"]:
            ...
            video_titles = {
                ...
                "kyjlxsLW1Is": "Statistics for Beginners",
                "UFZ3o6z3DlA": "Statistics in Nursing",
                "MynqBaooqus": "Medical Statistics Basics",
                "MunG48LH_hs": "Statistics for Marketing",
                "Bu7I_q9cYQ4": "A/B Testing Explained",
                "n8e3-vw_TbU": "Statistics for Various Fields",
                "9g80kqYyJ2o": "Healthcare Data Analysis"
            }
            
            video_title = video_titles.get(video_id, "Statistics in This Field")
            
            st.markdown(f"### 📺 Watch: {video_title}")
            st.markdown(f"""
            <div class='video-container'>
            <strong>Video Explanation:</strong>
            <div style="text-align: center; margin: 15px 0;">
            <a href="https://www.youtube.com/watch?v={video_id}" target="_blank" style="color: #667eea; font-weight: bold;">
            ▶️ Click here to watch on YouTube
            </a>
            </div>
            <p style="font-size: 0.9em; color: #666; margin-top: 10px;">
            This video shows how statistical methods are applied in {career_name.lower()} and similar fields.
            </p>
            </div>
            """, unsafe_allow_html=True)
        ...
        with st.expander("🔬 Top Real-World Statistics Examples by Field", expanded=True):
            ...
            with col1:
                ...
                st.markdown("• [Statistics in Nursing](https://www.youtube.com/watch?v=UFZ3o6z3DlA)")
                st.markdown("• [Medical Statistics Basics](https://www.youtube.com/watch?v=MynqBaooqus)")
            
            with col2:
                st.markdown("**Technology & Business:**")
                st.markdown("• [Statistics for Marketing](https://www.youtube.com/watch?v=MunG48LH_hs)")
                st.markdown("• [A/B Testing Explained](https://www.youtube.com/watch?v=Bu7I_q9cYQ4)")
            
            with col3:
                st.markdown("**Engineering & Science:**")
                st.markdown("• [Statistics for Engineers](https://www.youtube.com/watch?v=n8e3-vw_TbU)")
